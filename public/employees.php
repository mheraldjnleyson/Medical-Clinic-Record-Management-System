...
// Add Search Condition
    if (!empty($search)) {
        $where_search = "(name LIKE ? OR department LIKE ? OR position LIKE ? OR designation LIKE ? OR school_district_division LIKE ? OR entry_date LIKE ?)";
...
$allowed_sort = ['name', 'department', 'birth_date', 'gender', 'civil_status', 'first_year_in_service', 'school_district_division', 'position', 'designation'];
...
<section class="controls">
    <form method="GET" action="<?= BASE_PATH ?>employees" class="search-box" style="display:flex; align-items:center; width: 100%;">
    ...
    <a href="<?= BASE_PATH ?>generate_qr_employee" class="btn"
...
<div class="table-container" style="display: <?= (!empty($search) || isset($_GET['page'])) ? 'block' : 'none' ?>;">
    <table style="font-size: 11.5px;">
        <thead>
            <tr>
                <th onclick="sortTable('department')">Department <i
                        class="fa-solid <?= getSortIcon('department', $sort, $order) ?> th-filler"></i></th>
        ...
        fetch(`get_employee.php?id=${id}`)
            ...
            .then(data => {
                if (data.success) {
                    ...
                    document.getElementById('edit_school_district_division').value = "OCNHS";
