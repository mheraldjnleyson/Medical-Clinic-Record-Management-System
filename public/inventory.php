...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'edit_item') {
    ...
    $stmt = $conn->prepare("UPDATE inventory_items SET name = ?, category = ?, description = ?, unit = ?, expiry_date = ? WHERE id = ?");
    $stmt->bind_param("sssssi", $name, $category, $description, $unit, $expiry_date, $item_id);
...
elseif ($status_filter === 'expired')
    $where_clauses[] = "( (category != 'Equipment' AND expiry_date < CURDATE()) OR (category = 'Equipment' AND LOWER(expiry_date) LIKE '%non%') )";
...
$count_query = "SELECT COUNT(*) as total FROM inventory_items WHERE $where_sql";
...
$query = "SELECT *, IF(category != 'Equipment', DATEDIFF(expiry_date, CURDATE()), NULL) as days_to_expiry, (quantity <= 10 AND quantity > 0) as is_low_stock FROM inventory_items WHERE $where_sql ORDER BY name ASC LIMIT ? OFFSET ?";
...
<div class="inventory-dashboard"
    ...
    <?php
    $stats = [
        ['label' => 'Total Stock Items', 'val' => 0, 'icon' => 'fa-boxes-stacked', 'clr' => '#00ACB1'],
    ...
    $res_stats = $conn->query("SELECT 
        SUM(quantity > 0) as total_active_stock,
        SUM(quantity <= 10 AND quantity > 0) as low, 
        SUM(quantity = 0) as out_of,
        SUM((category != 'Equipment' AND expiry_date < CURDATE()) OR (category = 'Equipment' AND LOWER(expiry_date) LIKE '%non%')) as exp 
        FROM inventory_items WHERE is_archived = 0");
    if ($s = $res_stats->fetch_assoc()) {
        $stats[0]['val'] = $s['total_active_stock'] ?: 0;
        $stats[1]['val'] = $s['low'] ?: 0;
        $stats[2]['val'] = $s['exp'] ?: 0;
...
<section class="controls-bar"
    ...
    <div style="display: flex; align-items: center; gap: 10px;">
        <a href="<?= BASE_PATH ?>drug_log" class="btn-premium-action" style="height: 45px; padding: 0 25px;">
            <i class="fa-solid fa-clipboard-list"></i> DRUG LOG
        </a>
            ...
            <?php if ($result->num_rows > 0): ?>
                <?php while ($row = $result->fetch_assoc()): ?>
                    ...
                    if ($row['quantity'] == 0) {
                        $status = 'status-out';
                        $stext = 'Out of Stock';
                    } elseif ($row['category'] !== 'Equipment' && $row['expiry_date'] && $row['days_to_expiry'] !== null && $row['days_to_expiry'] < 0) {
                        $status = 'status-exp';
                        $stext = 'Expired';
                    } elseif ($row['category'] === 'Equipment' && stripos($row['expiry_date'], 'non') !== false) {
                        $status = 'status-exp';
                        $stext = 'Non-functional';
                    ...
                    <tr>
                        ...
                        <td style="font-weight: 600;">
                            <?php if ($row['category'] === 'Equipment'): ?>
                                <?= htmlspecialchars($row['expiry_date']) ?: '<span style="color:#dfe6e9">N/A</span>' ?>
                            <?php else: ?>
                        ...
                        <td style="text-align: center;">
                            <div style="display: flex; gap: 8px; justify-content: center;">
                                ...
                                <button
                                    onclick="openEditModal(<?= $row['id'] ?>, '<?= htmlspecialchars(addslashes($row['name'])) ?>', '<?= htmlspecialchars(addslashes($row['category'])) ?>', '<?= htmlspecialchars(addslashes($row['description'])) ?>', '<?= htmlspecialchars(addslashes($row['unit'])) ?>', '<?= $row['expiry_date'] ?>')"
                                    class="action-btn"
                                    style="background: rgba(243, 156, 18, 0.1); color: #f39c12; border:none; width:35px; height:35px; border-radius:8px; cursor:pointer;"
        ...
        <form method="POST" action="inventory.php">
            ...
            <div style="padding: 20px 0; display: flex; flex-direction: column; gap: 15px;">
                ...
                <div class="form-group"><label
                        ...
                        name="category" id="addCategory"
                        onchange="toggleExpiryCondition('addCategory', 'addExpiryLabel', 'addExpiryDate', 'addConditionInput')"
                ...
                <div class="form-group"><label id="addExpiryLabel"
                        style="font-weight:bold; display:block; margin-bottom:5px;">Expiry
                        Date</label>
                    <input type="date" name="expiry_date" id="addExpiryDate"
                        style="width:100%; padding:10px; border-radius:8px; border:1px solid #ddd;">
                    <input type="text" name="expiry_date" id="addConditionInput"
                        placeholder="e.g. Functional, Non-functional"
                        style="display:none; width:100%; padding:10px; border-radius:8px; border:1px solid #ddd;"
                        disabled>
                </div>
            </div>
            <div
...
<div id="editItemModal" class="modal-overlay" style="display: none;">
    <div class="modal-card" style="width: 500px;">
        <div class="modal-header"
            style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #eee; padding-bottom: 15px;">
            <h2 style="margin: 0; color: #f39c12;"><i class="fa fa-pen"></i> Edit Item</h2>
            <button class="close-btn" onclick="document.getElementById('editItemModal').style.display='none'"
                style="background: none; border: none; font-size: 1.5rem; cursor: pointer;">&times;</button>
        </div>
        <form method="POST" action="inventory.php">
            ...
            <div style="padding: 20px 0; display: flex; flex-direction: column; gap: 15px;">
                <div class="form-group"><label style="font-weight:bold; display:block; margin-bottom:5px;">Item Name
                        <span style="color:red">*</span></label><input type="text" name="name" id="editItemName"
                        required style="width:100%; padding:10px; border-radius:8px; border:1px solid #ddd;"></div>
                <div class="form-group"><label
                        style="font-weight:bold; display:block; margin-bottom:5px;">Category</label><select
                        name="category" id="editItemCategory"
                        onchange="toggleExpiryCondition('editItemCategory', 'editExpiryLabel', 'editItemExpiryDate', 'editConditionInput')"
                        style="width:100%; padding:10px; border-radius:8px; border:1px solid #ddd;">
                        <option>Medicine</option>
                        <option>Medical Supply</option>
                        <option>Equipment</option>
                ...
                <div class="form-group"><label
                        style="font-weight:bold; display:block; margin-bottom:5px;">Description</label><input
                        type="text" name="description" id="editItemDescription"
                        style="width:100%; padding:10px; border-radius:8px; border:1px solid #ddd;"></div>
                <div class="form-group"><label
                        style="font-weight:bold; display:block; margin-bottom:5px;">Unit</label><input type="text"
                        name="unit" id="editItemUnit" placeholder="pcs/bot"
                        style="width:100%; padding:10px; border-radius:8px; border:1px solid #ddd;"></div>
                <div class="form-group"><label id="editExpiryLabel"
                        style="font-weight:bold; display:block; margin-bottom:5px;">Expiry
                        Date</label>
                    <input type="date" name="expiry_date" id="editItemExpiryDate"
                        style="width:100%; padding:10px; border-radius:8px; border:1px solid #ddd;">
                    <input type="text" name="expiry_date" id="editConditionInput"
                        placeholder="e.g. Functional, Non-functional"
                        style="display:none; width:100%; padding:10px; border-radius:8px; border:1px solid #ddd;"
                        disabled>
                </div>
            ...
            <div
                style="display: flex; justify-content: flex-end; gap: 10px; padding-top: 15px; border-top: 1px solid #eee;">
                <button type="button" onclick="document.getElementById('editItemModal').style.display='none'"
                    style="padding: 10px 20px; border-radius:8px; border: 1px solid #ddd; background: #f8f9fa; cursor: pointer;">Cancel</button>
                <button type="submit"
                    style="padding: 10px 25px; border-radius:8px; border: none; background: #f39c12; color: white; font-weight: bold; cursor: pointer;">Update
                    Item</button>
            </div>
        </form>
    </div>
...
<script>
    ...
    function toggleExpiryCondition(catId, labelId, inputId, textInputId) {
        const cat = document.getElementById(catId).value;
        if (cat === 'Equipment') {
            document.getElementById(labelId).innerText = 'Condition';
            document.getElementById(inputId).style.display = 'none';
            document.getElementById(inputId).disabled = true;
            document.getElementById(textInputId).style.display = 'block';
            document.getElementById(textInputId).disabled = false;
        } else {
            document.getElementById(labelId).innerText = 'Expiry Date';
            document.getElementById(inputId).style.display = 'block';
            document.getElementById(inputId).disabled = false;
            document.getElementById(textInputId).style.display = 'none';
            document.getElementById(textInputId).disabled = true;
        }
    }
    ...
    function openEditModal(id, name, category, description, unit, expiry_date) {
        ...
        toggleExpiryCondition('editItemCategory', 'editExpiryLabel', 'editItemExpiryDate', 'editConditionInput');
        if (category === 'Equipment') {
            document.getElementById('editConditionInput').value = expiry_date ? expiry_date : '';
        } else {
            document.getElementById('editItemExpiryDate').value = expiry_date ? expiry_date : '';
        }
...
<style>
    ...
    body.dark-mode .inventory-dashboard {
        background: #18191a;
    }

    body.dark-mode .stat-glass-card {
        background: #242526 !important;
        border-color: #3a3b3c !important;
        box-shadow: 0 4px 15px rgba(0, 0, 0, 0.2) !important;
    }

    body.dark-mode .glass-text h3 {
        color: #e4e6eb !important;
    }

    body.dark-mode .glass-text p {
    ...
    body.dark-mode .search-box-unified {
        background: #3a3b3c !important;
        border-color: #4e4f50 !important;
    }

    body.dark-mode .search-box-unified input {
        color: #e4e6eb !important;
    }
    ...
    body.dark-mode .sub-desc {
        color: #b0b3b8 !important;
    }

    body.dark-mode .qty-bold {
        color: #e4e6eb !important;
    }

    body.dark-mode .page-btn-neo {
        background: #242526 !important;
        border-color: #3a3b3c !important;
        color: #e4e6eb !important;
    }

    body.dark-mode .page-btn-neo:hover {
    ...
    body.dark-mode td span[style*="color: #636e72"],
    body.dark-mode td[style*="color: #636e72"] {
        color: #b0b3b8 !important;
    }

    body.dark-mode .action-btn {
        background: rgba(0, 172, 177, 0.2) !important;
    }

    body.dark-mode .action-btn[style*="color: #e74c3c"] {
    ...
    @media screen and (max-width: 768px) {
        ...
        .controls-bar .btn-premium-action,
        .controls-bar .dropdown,
        .controls-bar .dropdown button,
        .controls-bar .btn-add-item {
            width: 100% !important;
