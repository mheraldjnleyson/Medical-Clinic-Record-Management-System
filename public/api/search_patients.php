...
if (empty($typeFilter) || $typeFilter === 'employee') {
    $stmt2 = $conn->prepare("SELECT id, name, department, gender, birth_date, address, position, 'Employee' as type FROM employees WHERE (name LIKE ? OR department LIKE ? OR address LIKE ?) AND is_archived = 0 LIMIT 10");
