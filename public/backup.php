...
if (isset($_GET['action']) && $_GET['action'] == 'backup') {
  // CLEANUP: Delete any temp zip files in the backups folder older than 1 hour
  ...
  // ZIP Creation Path - Using project backups directory for better reliability
  $backupDir = __DIR__ . DIRECTORY_SEPARATOR . 'backups';
  if (!is_dir($backupDir)) {
  ...
  $zip = new ZipArchive();
  if ($zip->open($zipPath, ZipArchive::CREATE | ZipArchive::OVERWRITE) !== TRUE) {
    die("Failed to create ZIP archive at $zipPath. Please check folder permissions.");
  }
  ...
  if ($backupType == 'essential') {
    // ESSENTIAL MODE: Database + Any 'uploads' or 'archives' files
    $backupLog = "Backup Type: Essential\nGenerated: " . date('Y-m-d H:i:s') . "\n";

    $zip->addEmptyDir('uploads');
    $zip->addEmptyDir('archives');

    // Scan project recursively but SKIP known large system folders for speed
    $it = new RecursiveDirectoryIterator($projectRoot, RecursiveDirectoryIterator::SKIP_DOTS);

    // Optimized Filter: Skip .git, node_modules, and other system folders
    $filter = new RecursiveCallbackFilterIterator($it, function ($current, $key, $iterator) {
      $skip = ['.git', 'node_modules', 'backups', 'logs', 'tmp', 'temp'];
    ...
    foreach ($files as $file) {
      ...
      // Case-insensitive check for /uploads/ or /archives/
      $isUpload = (stripos($normalizedPath, '/uploads/') !== false);
      $isArchive = (stripos($normalizedPath, '/archives/') !== false);

      if ($isUpload || $isArchive) {
        if ($isUpload) {
          $parts = preg_split('/\/uploads\//i', $normalizedPath);
          $inZipPath = 'uploads/' . end($parts);
        } else {
          $parts = preg_split('/\/archives\//i', $normalizedPath);
          $inZipPath = 'archives/' . end($parts);
        }

        if ($zip->addFile($filePath, $inZipPath)) {
          $fileCount++;
        } else {
          $backupLog .= "Error adding file: $filePath\n";
        }
      }
    }
    $backupLog .= "Total files added to backup: $fileCount\n";
    $zip->addFromString('backup_info.txt', $backupLog);
  } else {
  ...
  require_once '../config/backup_reminder.php';
  logBackup($conn, $_SESSION['username']);
  logSecurityEvent('DATABASE_BACKUP', 'User ' . $_SESSION['username'] . ' created a system backup.');
  $_SESSION['backup_created'] = true;

  // Download ZIP
  if (file_exists($zipPath)) {
    // Clear all buffers to ensure clean ZIP download
    while (ob_get_level()) {
      ob_end_clean();
    }
    ...
    unlink($zipPath); // Delete temp file from backups directory
    exit;
  } else {
    die("Error: Backup file was not created. Please check if the 'public/backups' folder exists and is writable.");
  }
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['backup_file'])) {
  ...
  if ($file_ext === 'sql') {
    ...
    if ($zip->open($file_tmp) === TRUE) {
      ...
      // 3. Restore Archives
      $archivesSource = $extractPath . '/archives';
      $archivesTarget = realpath(__DIR__ . '/../archives');
      if (file_exists($archivesSource) && $archivesTarget) {
        $files = new RecursiveIteratorIterator(
          new RecursiveDirectoryIterator($archivesSource, RecursiveDirectoryIterator::SKIP_DOTS),
          RecursiveIteratorIterator::SELF_FIRST
        );
        foreach ($files as $item) {
          $subPath = $files->getSubPathName();
          $destPath = $archivesTarget . DIRECTORY_SEPARATOR . $subPath;
          if ($item->isDir()) {
            if (!file_exists($destPath))
              mkdir($destPath, 0755, true);
          } else {
  ...
  header("Location: " . BASE_PATH . "backup");
  exit;
}
    ...
    <div class="card-content">
      ...
      <form action="backup.php" method="POST" enctype="multipart/form-data">
        ...
        <button type="submit" class="btn red w-100"
          onclick="return confirm('CRITICAL WARNING: Restoring will PERMANENTLY OVERWRITE all current database records and files. This action cannot be undone. Are you sure you want to proceed?')">
