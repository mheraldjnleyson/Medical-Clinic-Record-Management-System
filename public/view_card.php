...
<head>
  ...
  <style>
    @media print {
      ...
      /* Relaxed spacing to fill more of the page height for Long Bond Paper */
      .info-row {
            ...
            <tr>
              ...
              <?php for ($g = 7; $g <= 12; $g++) {
                ...
                if ($onlyDate) {
                  // Only Display Date
                  echo $dateVal;
                  // Special case: Display Menarche value below date if it exists
                  if ($row === 'Menarche' && $val) {
                    echo "<div style='font-size:9px; margin-top:2px;'>" . $val . "</div>";
                  }
                  // Special case: Display Immunization remarks below date if it exists
                  $remarksVal = getVal($slug . '_remarks_' . $g, $health_values);
                  if ($row === 'Immunization (specify)' && $remarksVal) {
                    echo "<div style='font-size:9px; margin-top:2px;'>" . $remarksVal . "</div>";
                  }
                } else {
                  // Display Main Value
