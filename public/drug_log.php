...
$res = $conn->query("SELECT name, department, treatment_logs_json FROM employees WHERE is_archived = 0");
while ($row = $res->fetch_assoc()) {
    $logs = json_decode($row['treatment_logs_json'] ?? '[]', true);
    $allMedLogs = array_merge($allMedLogs, extractMedicationLogs($logs, $row['name'], 'Employee', $row['department'], $startDate, $endDate, $searchMed));
}
...
if ($searchMed) {
    $transQuery .= " AND (i.name LIKE ? OR t.remarks LIKE ?)";
...
<style>
    ...
    @media screen and (max-width: 768px) {
        ...
        .hero-right, .dropdown-wrapper, .btn-premium {
            width: 100%;
        }
        .btn-premium {
            justify-content: center;
