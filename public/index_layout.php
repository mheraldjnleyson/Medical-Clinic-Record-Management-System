...
// Base Path is now handled centrally in db.php via BASE_PATH constant
$basePath = BASE_PATH;
...
<head>
  ...
  <link rel="icon" type="image/x-icon" href="<?= $basePath ?>assets/img/ocnhs_logo.png">
  <link rel="stylesheet" href="<?= $basePath ?>assets/css/style.css">
  <link rel="stylesheet" href="<?= $basePath ?>assets/css/dropdown.css?v=<?= time() ?>">
  <link rel="stylesheet" href="<?= $basePath ?>assets/css/responsive.css">
  <link rel="stylesheet" href="<?= $basePath ?>assets/css/dark-mode.css?v=<?= time() ?>">
  ...
  <?php include __DIR__ . '/assets/inc/console_suppress.php'; ?>
...
<body class="page-<?= str_replace('.php', '', $currentPage) ?>">
    ...
    if (shouldShowBackupReminder($conn)) {
      ...
      <div id="persistentBackupReminder" class="persistent-banner">
        <div class="banner-content">
          ...
          <a href="' . $basePath . 'backup.php?action=backup&type=essential" class="banner-btn" onclick="hideBackupBanner()"><i class="fa-solid fa-download"></i> Go to Backup</a>
        </div>
      </div>
      <script>
        function hideBackupBanner() {
          const banner = document.getElementById("persistentBackupReminder");
  ...
  <header>
    <div class="logo">
      <a href="<?= $basePath ?>student"> <img src="<?= $basePath ?>assets/img/LOGO.png" alt="Logo"></a>
    </div>
    ...
    // Expired Items
    $invExpSql = "SELECT id, name, category, expiry_date FROM inventory_items WHERE (category != 'Equipment' AND expiry_date IS NOT NULL AND expiry_date <= CURDATE()) OR (category = 'Equipment' AND LOWER(expiry_date) LIKE '%non%')";
    ...
    if ($invExpRes) {
      while ($row = $invExpRes->fetch_assoc()) {
        ...
        $is_equip = ($row['category'] === 'Equipment');
        $notifications[] = [
          'type' => 'inventory_expired',
          'title' => $is_equip ? 'Item Non-functional' : 'Item Expired',
          'message' => $is_equip ? $row['name'] . " is marked as " . $row['expiry_date'] : $row['name'] . " expired on " . date('M d, Y', strtotime($row['expiry_date'])),
    ...
    <div class="header-right">
      ...
      <div class="user-dropdown" onclick="toggleUserMenu()">
        ...
        <div class="dropdown-menu" id="userMenu">
          <a href="<?= $basePath ?>profile"><i class="fa-solid fa-user-gear"></i> PROFILE</a>
          <a href="<?= $basePath ?>cv.php"><i class="fa-solid fa-file-invoice"></i> DEVELOPER CV</a>
          <a href="<?= $basePath ?>change_password"><i class="fa-solid fa-key"></i> PASSWORD</a>
          <?php if (isset($_SESSION['role']) && ($_SESSION['role'] === 'admin' || $_SESSION['role'] === 'superadmin')): ?>
            <a href="<?= $basePath ?>archive_list"><i class="fa-solid fa-box-archive"></i> ARCHIVE LIST</a>
          <?php endif; ?>
          <hr>
          <a href="<?= $basePath ?>logout" class="logout-link"><i class="fa-solid fa-right-from-bracket"></i> SIGN
            OUT</a>
        </div>
      </div>
    </div>
  ...
  <nav>
    <a href="<?= $basePath ?>dashboard"
      class="<?= ($currentPage == 'dashboard.php' || $currentPage == 'dashboard') ? 'active' : '' ?>">
      <span>Dashboard</span>
    </a>

    <a href="<?= $basePath ?>student"
      class="<?= ($currentPage == 'student.php' || $currentPage == 'student') ? 'active' : '' ?>">
      <span>Students Records</span>
    </a>

    <a href="<?= $basePath ?>employees"
      class="<?= ($currentPage == 'employees.php' || $currentPage == 'employees') ? 'active' : '' ?>">
      <span>Employee Records</span>
    </a>

    <a href="<?= $basePath ?>others"
      class="<?= ($currentPage == 'others.php' || $currentPage == 'others') ? 'active' : '' ?>">
      <span>Others Records</span>
    </a>

    <a href="<?= $basePath ?>inventory"
      class="<?= ($currentPage == 'inventory.php' || $currentPage == 'inventory') ? 'active' : '' ?>">
      <span>Inventory</span>
    </a>

    <?php if (isset($_SESSION['role']) && ($_SESSION['role'] === 'admin' || $_SESSION['role'] === 'superadmin')): ?>
      <a href="<?= $basePath ?>logs"
        class="<?= ($currentPage == 'logs.php' || $currentPage == 'logs') ? 'active' : '' ?>">
        <span>Activity Logs</span>
      </a>

      <a href="<?= $basePath ?>users"
        class="<?= ($currentPage == 'users.php' || $currentPage == 'users') ? 'active' : '' ?>">
        <span>User Management</span>
      </a>

      <a href="<?= $basePath ?>merge_tool"
        class="<?= ($currentPage == 'merge_tool.php' || $currentPage == 'merge_tool') ? 'active' : '' ?>">
        <span>Merge Records</span>
      </a>

      <a href="<?= $basePath ?>backup"
        class="<?= ($currentPage == 'backup.php' || $currentPage == 'backup') ? 'active' : '' ?>">
        <span>Backup Recovery</span>
      </a>
    <?php endif; ?>

    <?php if (isset($_SESSION['role']) && $_SESSION['role'] === 'doctor'): ?>
      <a href="<?= $basePath ?>certificate_generator"
        class="<?= ($currentPage == 'certificate_generator.php') ? 'active' : '' ?>">
              ...
              if (!Swal.isVisible()) {
                Swal.fire({
                  ...
                  if (result.isConfirmed) {
                    window.location.href = '<?= BASE_PATH ?>?reason=concurrent_login';
                  }
                });
              }
  ...
  <script>
    function markAllNotificationsRead(e) {
      ...
      fetch('<?= isset($basePath) ? $basePath : "" ?>api/mark_all_read.php', {
        method: 'POST'
      })
        .then(response => response.json())
        .then(data => {
