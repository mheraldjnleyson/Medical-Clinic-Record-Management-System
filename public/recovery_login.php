...
// Local base path for recovery pages (db.php may not be loaded)
if (!defined('BASE_PATH')) {
    $_bp = str_replace(basename($_SERVER['SCRIPT_NAME']), '', $_SERVER['SCRIPT_NAME']);
    if (strpos($_bp, '/public/') !== false) $_bp = str_replace('/public/', '/', $_bp);
    $_bp = rtrim($_bp, '/') . '/';
    define('BASE_PATH', $_bp);
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (verifyRecoveryCredentials($username, $password)) {
        enableRecoveryMode();
        header("Location: " . BASE_PATH . "recovery_panel");
        exit();
    } else {
